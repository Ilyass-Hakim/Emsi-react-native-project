...
export const FirebaseService = {
    ...
    async createIncident(incidentData) {
        const docRef = await addDoc(collection(db, 'incidents'), {
            ...
            statusHistory: [{
                ...
                evidenceUrl: incidentData.images && incidentData.images.length > 0 ? incidentData.images[0] : null
            }],
            createdAt: serverTimestamp(),
            updatedAt: serverTimestamp(),
        });
    ...
    async updateIncidentStatus(incidentId, newStatus, note, user, imageUrl = null) {
        const incidentRef = doc(db, 'incidents', incidentId);
        const incidentSnap = await getDoc(incidentRef);
        if (incidentSnap.exists()) {
            const history = incidentSnap.data().statusHistory || [];
            ...
            if (imageUrl) {
                newHistoryItem.evidenceUrl = imageUrl;
            }
    ...
    async addIncidentEvidence(incidentId, imageUrl, user, note = 'Additional proof uploaded') {
        const incidentRef = doc(db, 'incidents', incidentId);
        const incidentSnap = await getDoc(incidentRef);
        if (incidentSnap.exists()) {
            const history = incidentSnap.data().statusHistory || [];
            ...
            const newHistoryItem = {
                status: incidentSnap.data().status, // Status doesn't change
                note: note,
                user: user || 'Responder',
                evidenceUrl: imageUrl,
                type: 'evidence', // helpful for UI to distinguish
                timestamp: new Date().toISOString()
            };

            const currentImages = incidentSnap.data().evidenceImages || [];

            await updateDoc(incidentRef, {
                statusHistory: [...history, newHistoryItem],
                evidenceImages: [...currentImages, imageUrl],
                updatedAt: serverTimestamp()
            });
        }
    },
    ...
    subscribeToAnalytics(callback, onError) {
        // Subscribe to incidents to calculate metrics
        // Ideally this should be a cloud function or pre-aggregated stats doc, 
        // but for this MVP we'll aggregate client-side
        const q_incidents = query(collection(db, 'incidents'), orderBy('createdAt', 'desc'));
        const q_users = query(collection(db, 'users'));

        // We need to listen to both. For simplicity, we'll listen to incidents 
